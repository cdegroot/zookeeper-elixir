defmodule Zookeeper.WriteLock do
  ...
  @doc """
  Obtain a lock using the indicated path and then call passed in function. Returns the result
  ...
  Note that you _must_ pass a timeout. I think it's good practice to make this explicit.
  """
  def lock(zk, path, timeout, fun) do
    {:ok, pid} = GenServer.start_link(__MODULE__, {zk, path, timeout, fun})
    try do
  ...
  def init({zk, path, timeout, fun}) do
    ...
    {:ok, %{zk: zk, path: path, timeout: timeout,
            fun: fun, node: node, return_pid: nil}}
  end
  ...
  defp try_lock(%{zk: zk, path: path, timeout: timeout, fun: fun, node: node}=state) do
    # Find children
